...
class Log_aktivitas extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('t_log_aktivitas');
	...
	public function index()
	{
		...
		$data = array(
			...
			'data_user' => $data_user,
			'list_menu' => $this->t_log_aktivitas->get_menu_by_log_aktivitas(),
			'list_aksi' => ['create', 'update', 'delete', 'login', 'logout']
		);
		...
		$content = [
			...
			'modal' => null,
			'js'	=> 'log_aktivitas.js',
			'view'	=> 'view_log_aktivitas'
		];

		$this->template_view->load_view($content, $data);
	}

	public function list_log_aktivitas()
	{
		...
		$list = $this->t_log_aktivitas->get_datatable_log($tgl_awal, $tgl_akhir, $menu, $aksi);
			...
			$str_aksi = '
				<div class="btn-group">
					...
					<div class="dropdown-menu">
						<button class="dropdown-item" onclick="detailLog(\''.$item->id.'\')">
		...
		$output = [
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->t_log_aktivitas->count_all($tgl_awal, $tgl_akhir, $menu, $aksi),
			"recordsFiltered" => $this->t_log_aktivitas->count_filtered($tgl_awal, $tgl_akhir, $menu, $aksi),
			"data" => $data
		];
